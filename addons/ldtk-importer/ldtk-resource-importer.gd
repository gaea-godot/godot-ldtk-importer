...
func save_world(world: LDTKWorldData, save_path: String, gen_files: Array[String]) -> Error:
	...
	var err = ResourceSaver.save(world, world_path)
	if err == OK:
...
func save_level(level: LDTKLevel, save_path: String, gen_files: Array[String]) -> String:
	...
	var level_path = (
		...
		% [
			save_path,
			level.name,
			LevelSaveExtensions.keys()[Util.options.level_save_extension].to_lower()
		]
	)
